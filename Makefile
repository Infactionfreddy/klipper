...
OBJS_klipper.elf = $(sort $(patsubst %.c, $(OUT)src/%.o,$(src-y))) $(OUT)compile_time_request.o
...
$(OUT)compile_time_request.o: $(patsubst %.c, $(OUT)src/%.o.ctr,$(src-y)) ./scripts/buildcommands.py
	@echo "  Building $@"
	$(Q)cat $(sort $(patsubst %.c, $(OUT)src/%.o.ctr,$(src-y))) | tr -s '\0' '\n' > $(OUT)compile_time_request.txt
...
$(OUT)autoconf.h: $(KCONFIG_CONFIG)
	...
	$(Q)mkdir -p $(OUT)
	$(Q)if [ ! -f $@ ]; then \
		KCONFIG_AUTOHEADER=$@ $(PYTHON) lib/kconfiglib/genconfig.py src/Kconfig || touch $@; \
	fi

$(KCONFIG_CONFIG) olddefconfig: src/Kconfig
	@echo "Skipping Kconfig (using pre-configured values)"
